...
function getQuery(type) {
    ...
    switch(type) {
        case "allThreads":
            ...
            break;
        case "threadById":
            query = "SELECT m.message_id, m.thread_id, u.first_name, u.last_name, m.post, DATE_FORMAT(m.date, '%M-%D-%Y') as date FROM messages m INNER JOIN user u ON m.user_id = u.user_id WHERE m.thread_id = ? ORDER BY m.date ASC;"
    }
    return query;
};
